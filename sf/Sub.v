...
(** **** Exercise: 2 stars, standard, recommended (arrow_sub_wrong)  
    ...
    f : \s: Student. s.gpa
    g : \f: Person -> Nat. f { name="foo", age=42 }

    Can't reduce the term { name="foo", age=42 }.gpa that arises from
...
(** **** Exercise: 1 star, standard, optional (subtype_instances_tf_1)  
    ...
    - [Top->U <: S->Top] true (S_Arrow)

    - [(C->C) -> (A*B)  <:  (C->C) -> (Top*B)] 
      true, A <: Top (S_Top), (A*B) <: (Top*B) (S_Prod),
        so x -> (A*B) <: x -> (Top*B) (S_Arrow)

    - [T->T->U <: S->S->V] 
      true (T->U) <: (S->V) (S_Arrow), and (T -> x) <: (S -> y) 
      if x <: y (S_Arrow)

    - [(T->T)->U <: (S->S)->V] 
      false, because (T->T) <: (S->S) doesn't hold

    - [((T->S)->T)->U <: ((S->T)->S)->V]
      true:
      x1 -> U <: y1 -> V iff y1 <: x1 
      with y1 = ((S->T)->S), x1 = ((T->S)->T)
      x2 -> S <: y2 -> T iff y2 <: x2
      with y2 = (S -> T), x2 = (T -> S)
      this is true (S_Arrow), so the whole lemma holds
...
let TTS = Top -> Top -> Student and TS = Top -> Student in 
  TTS <: Top
  Top -> Student, unrelated to TTS, because Student is unrelated to TS
  Student -> Person, unrelated, idem
  TTS <: Student -> Top because Student <: Top and TS <: Top
  Person -> Student unrelated to TTs becase Student is unrelated to TS
*)
...
(** **** Exercise: 1 star, standard (subtype_instances_tf_2)  
    ...
    _false_ after each one.
      ...
      forall S T,
          S <: T  ->
          S->S   <:  T->T
      false, we would need T <: S for contravariance of the input

      forall S,
           S <: A->A ->
           exists T,
              S = T->T  /\  T <: A
      false, we would also need A <: T for contravariance of the input
      ...
      exists S,
           S <: S->S 
      false, if we suppose that this is true, then there exists 
      S1 and S2 such as S = S1 -> S2 /\ S <: S1 /\ S2 <: S

      exists S,
           S->S <: S  
      true, Top satisfies this proposition

      forall S T1 T2,
           S <: T1*T2 ->
           exists S1 S2,
              S = S1*S2  /\  S1 <: T1  /\  S2 <: T2  
      true, trivially deduced from S_Prod
*)
...
(** **** Exercise: 1 star, standard (subtype_concepts_tf)  
    ...
    - There exists a type that is a subtype of every other type.
      false (unless we add bottom to our rules)
    ...
    - There is an infinite descending chain of distinct types in the
      ...
      true, it is defined as S0 = Top, S(i+1) = Top -> Si
      Top -> Top <: Top 
      Top -> (Top -> Top) <: Top -> Top (covariance of return type)
      ...
...
(** **** Exercise: 2 stars, standard (proper_subtypes)  
    ...
    forall T,
         ...
         exists S,
            S <: T  /\  S <> T
    rephrased, assuming that T is neither Bool nor a supertype, there
    exists an S different from T, which is a subtype of T. 
    false, T is not a supertype so there generally is no S <: T, 
    except T itself
*)
...
(** **** Exercise: 3 stars, standard, optional (count_supertypes)  
    ...
    Top, {}, {x:A}, {y:C->C}, {y:C->C, x:A}, 
    {y: C->Top}, {y: C-> Top, x: A} {x: A, y: C->Top}, 
    {y: Top}, {y: Top, x:A}, {x:A, y: Top}.
    + same with A <: Top... = 11 + 6 = 17
    [] *)
...
(** **** Exercise: 2 stars, standard (pair_permutation)  
    ...
    terrible idea in the context of functions. We could easily producte
    mismatched inputs or outputs
*)
...
Proof with eauto.
  eapply S_Arrow... apply sub_student_person.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  + apply IHHs2 in HeqV as [U1 [U2 [HAU [HU1 HU2]]]].
    apply IHHs1 in HAU as [U3 [U4 [HAU' [HU3 HU4]]]].
    exists U3, U4...
