...
(* the store contains something like [c1.c = 0, c2.c = 0, c1 = { i= (c1.c += 1), d = (c1.c -= 1) }, c2 = { i = (c2.c += 1), d = (c2.c -= 1) } ]
...
would it behave the same? 
  no, since in the second one, 'a' would be dereferenced _after_ 
  reassignment. Call to (!a) n with n /= m would result in an 
  infinite loop
*)
...
(*  Using some imaginary syntax where 'delete f' simply removes
    the value with 'f' key from the context.
    let f = (\x:Nat.x) in
    a := f; -- 'a' has type (Ref (Arrow Nat Nat))
    delete f;;
    let f = 0 in
    (!a) f  -- this will reduce to (f f), which is ill-typed
*)
...
Definition fact_impl : tm :=
  abs y Nat
    (test0 (var y)
      (const 1)
      (mlt (var y) (app (deref (var r)) (prd (var y))))).

Definition factorial : tm :=
  (abs y Nat 
    (app
      (abs r (Ref (Arrow Nat Nat))
        (tseq (assign (var r) fact_impl)
                (app (deref (var r)) (var y))))
      (ref (abs y Nat (var y))))).

Lemma factorial_type : empty; nil |- factorial \in (Arrow Nat Nat).
Proof with eauto.
  unfold factorial, fact_impl...
  eapply T_Abs.
  eapply T_App. eapply T_Abs. unfold tseq. eapply T_App.
  eapply T_Abs. eapply T_App. eapply T_Deref.
  eapply T_Var. unfold update, t_update... rewrite false_eqb_string.
  simpl. reflexivity. discriminate. apply T_Var. unfold update, t_update.
  rewrite false_eqb_string... discriminate. eapply T_Assign. eapply T_Var.
  unfold update, t_update... reflexivity. eapply T_Abs. eapply T_If0...
  eapply T_Mult... eapply T_App... eapply T_Deref... eapply T_Ref...
Qed.
