...
Inductive listpair := lpair (l1: natlist) (l2: natlist).
...
Fixpoint alternate (l1 l2 : natlist) : natlist := 
  match l1, l2 with
    ...
    | nil, l => l
    | l, nil => l
    | cons n ns, cons m ms => ( n :: m :: alternate ns ms )
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | cons n ns => match n =? v with
    | true => ns
    | false => n :: remove_one v ns
    end
  end.
...
| nil => nil
| cons n ns => match n =? v with
  | true => remove_all v ns
  | false => n :: remove_all v ns
  end
end.
...
match s1 with
| nil => true
| cons n ns => let r := remove_one n s2 in
  (length r =? (length s2) - 1) && (subset ns r)
end.
...
Proof. induction n as [| n HN].
  + reflexivity.
  + simpl. rewrite HN. reflexivity.
Qed.

Theorem bag_count_add_S : forall (b: bag) (n: nat), 
...
Theorem bag_app_nil_r : forall (b: bag),
  b ++ nil = b.
Proof. induction b as [| n ns IH ].
 + reflexivity.
 + simpl. rewrite IH. reflexivity.
Qed.

Theorem bag_count_eqb_S : forall (n: nat) (m: nat) (b: bag),  
  (n =? m) = true -> count n (m :: b) = S (count n b).
Proof.
  intros n m b H.
  simpl. rewrite H. reflexivity.
Qed.

Theorem bag_count_neqb : forall (n: nat) (m: nat) (b: bag),
  (n =? m) = false -> count n (m :: b) = count n b.
Proof.
  intros n m b H.
  simpl. rewrite H. reflexivity.
Qed.

Theorem bag_count_app_plus : forall (b1: bag) (b2: bag) (n: nat),
  (count n (b1 ++ b2)) = count n b1 + count n b2.
Proof.
  intros b1 b2 n.
  induction b1 as [| c cs HC ], b2 as [| d ds ].
  + reflexivity.
  + reflexivity.
  + rewrite bag_app_nil_r. simpl. rewrite <- plus_n_O. reflexivity.
  + simpl. rewrite HC. 
    destruct (n =? d) eqn: D.
    - rewrite bag_count_eqb_S. 
      assert (I: S (count n cs + S (count n ds)) = 
                 S (count n cs) + S (count n ds)). 
      { reflexivity. }
      rewrite I. destruct (n =? c).
        * reflexivity.
        * reflexivity.
        * rewrite D. reflexivity.
    - rewrite bag_count_neqb. destruct (n =? c).
      * reflexivity.
      * reflexivity.
      * rewrite D. reflexivity.
Qed.
...
Proof.
  intros l1 l2.
  induction l1 as [| n ns H ].
  + simpl. rewrite app_nil_r. reflexivity.
  + simpl. rewrite H, app_assoc. reflexivity.
  Qed. 

Theorem rev_single_refl : forall m,
  rev [m] = [m].
Proof. reflexivity. Qed.
...
Proof.
  induction l as [| n ns H ].
  + reflexivity.
  + simpl. rewrite <- rev_single_refl. 
    replace (rev (rev ns ++ rev [n])) with 
      (rev (rev [n]) ++ rev (rev ns)).
    - rewrite H. rewrite ?rev_single_refl. reflexivity.
...
Proof.
  intros l1 l2.
  induction l1 as [| l ls H ].
  + reflexivity.
  + destruct l as [| n] eqn: D.
    - simpl. rewrite H. reflexivity.
    - simpl. rewrite H. reflexivity. 
Qed.
(** [] *)
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | nil, nil => true
  | cons n ns, cons m ms => (n =? m) && (eqblist ns ms)
  | _, _ => false
  end.
...
Proof.
  induction l as [| n ns H ].
  + reflexivity.
  + simpl. rewrite eqb_refl. rewrite <- H. reflexivity.
Qed.
(** [] *)
...
Proof.
  induction s as [| n ns H ].
  + reflexivity.
  + destruct n as [| n' ] eqn: D.
...
(* 0. It cannot be constructed as all its constructors require a pre-existing baz. Since no baz exists at the point of declaration, there is simply no way to ever create one. *)
