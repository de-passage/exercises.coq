...
Proof.
  ...
  inversion H1. exfalso. apply H. reflexivity.
Qed.
...
Proof.
  ...
  + inversion H. reflexivity. apply le_trans with 1. 
    - apply le_S. reflexivity.
    - apply H1.
  + inversion H. apply le_n. apply le_trans with (S (S n)).
...
(** **** Exercise: 2 stars, standard, optional (le_Sn_n_inf)  
    ...
    Proof: 
    Let's prove that for all n, n + 1 <= n is False. Let's start by 
      assuming that the hypothesis (n + 1 <= n) holds for all n.
    By induction over n:
    + if n = 0, then we have supposed that 1 <= 0, a contradiction with the
      definition of le. 
    + if n = S n', then we have supposed that n + 1 <= n doesn't hold and 
      must prove that n + 2 <= n + 1 doesn't hold. 
      There are two cases in which n + 2 <= n + 1 holds,
      - either n + 2 = n + 1, which is a contradiction
      - there is a m such as m + 1 = n + 1, i.e. n = m, and n + 2 <= m, 
        i.e. n + 2 <= m.
        Since n + 1 <= n + 2 (by definition of le: n+1 = n+1 and S(n+1) = n+2), 
        then by transitivity, n + 1 <= m, which we have assumed is false in our
        induction hypothesis.
      Both case are contradictions, therefore n + 2 <= n + 1 doesn't hold.
...
Proof.
  ...
  + inversion H.
  + inversion H. 
    - apply IHn. rewrite H1 in H. apply H.
...
Proof.
  ...
  intros H. assert (A: 0 <= 1). 
  + apply le_S. apply le_n.
  + apply H in A. inversion A.
Qed.
(** [] *)
...
Proof.
  ...
  - intros b H1 H2. inversion H1. 
    * reflexivity.
    * apply f_equal. apply IHa.
      + apply le_Sn_le. apply H.
      + rewrite <- H0 in H2. apply le_S_n. apply H2.
Qed.
(** [] *)
...
Proof.
  ...
  induction H1 as [| x y z IHR IHcR IHcrt ].
  + intros a H2. apply H2.
  + intros a H2. apply rt1n_trans with y. 
    * apply IHR.
    *  apply IHcrt in H2. apply H2.
Qed.
(** [] *)
