...
Proof.
  ...
  + simpl. rewrite HX. rewrite app_assoc. reflexivity.
Qed.
...
Proof.
  ...
  + reflexivity.
  + simpl. assert (I: rev [x] = [x]). { reflexivity. }
    rewrite <- I. rewrite rev_app_distr. rewrite H.
...
match l with
| nil => ([], [])
| cons (x, y) r => let (xs, ys) := split r in
  (x :: xs, y :: ys)
end.
...
Proof.
  ...
  induction l as [| x xs H ].
  + reflexivity.
  + simpl. rewrite <- H. rewrite -> map_app_distr.
    reflexivity.
Qed.
(** [] *)
...
(* Map to another monoid when doing the operation. e.g. 
any {X : Type} (p : X -> bool) (l: list X) := 
  fold (fun x a => (p x) || a) l false.
*)
...
(** **** Exercise: 2 stars, advanced (nth_error_informal)  
   ...
   By induction over l
    + if l = [], nth_error l n = None by definition
    + if l = (x :: xs), we need to prove nth_error (x :: xs) n = None
      By definition of length, there exists an n', such as n = S n'.
      By definition, nth_error (x :: xs) (S n') = nth_error xs n'
      And we'll pretend that that's convincing enough for induction
...
Definition exp (n m : cnat) : cnat :=
  fun (X: Type) (f: X -> X) (x: X) => 
    m (X -> X) (fun (g: X -> X) (y: X) => n X g y) f x.
