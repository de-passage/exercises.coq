...
Theorem plus_comm : forall n m : nat,
  n + m = m + n.
Proof.
  intros n m. induction n as [| n' E].
  + rewrite <- plus_n_O. reflexivity.          
  + simpl. rewrite <- plus_n_Sm. rewrite <- E. reflexivity.
  Qed.
...
Proof.
  ...
  + destruct n' as [| n''] eqn:H'.
    - reflexivity.
    - rewrite -> H. rewrite -> negb_involutive. rewrite <- H'. simpl. reflexivity.
  Qed.
(** [] *)
...
(* destruct is basically pattern matching over the different constructors of a type. induction is the decomposition of a base case and the pair of a value and the assumption that a statements holds for said value. They are related in that many types are defined in an inductive manner: with a base case and a constructor producing a new value from an existing value of the same type. They are different in that destruct doesnt make any assumption on the validity of the original statement.
*)
*)
